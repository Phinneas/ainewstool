...
async function fetchJsonFeed(
  url: string
): Promise<{ items: RawJsonFeedItem[] }> {
  const response = await fetch(url);
  if (!response.ok) throw new Error(`Failed to fetch ${url}: ${response.status}`);
  return (await response.json()) as { items: RawJsonFeedItem[] };
}
...
async function fetchJsonFeedItems(
  feed: FeedDefinition
): Promise<NormalizedFeedItem[]> {
  try {
    ...
    if (feed.name === "google_news") {
      return data.items.map((item) =>
        normalizeGoogleNewsItem(item, feed.feedUrl)
      );
    }
    return data.items.map((item) =>
      normalizeJsonFeedItem(item, feed.sourceName, feed.feedType, feed.feedUrl)
    );
  } catch (err) {
...
async function fetchRedditFeedItems(
  feed: FeedDefinition
): Promise<NormalizedFeedItem[]> {
  try {
    const data = await fetchJsonFeed(feed.httpUrl!);
    const processed = await processRedditFeedItems(
      data.items.map((item) => ({
        url: item.url ?? "",
        title: item.title,
      })),
...
async function fetchAllFeeds(): Promise<NormalizedFeedItem[]> {
...
async function stageUpload(items: EvaluatedItem[]): Promise<number> {
