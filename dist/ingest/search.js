 ...
 */
export async function fetchShowHN(query, maxResults) {
    try {
        ...
        const results = (data.hits || []).filter((h) => h.points >= 3).map((h) => ({
            title: h.title,
...
function parallelHeaders(extra = {}) {
    return {
        "Content-Type": "application/json",
 ...
 */
export async function parallelSearch(queries, category, limit = 5) {
 ...
 */
export async function parallelFindAll(objective, matchLimit = 20) {
    if (!config.parallel?.apiKey) {
    ...
    try {
                ...
                match_conditions: [
                    {
                        ...
                        description: "Built or launched by an indie developer, small team, or startup " +
        ...
        const { findall_id } = (await createRes.json());
        ...
        while (Date.now() + 5_000 < deadline) {
            ...
            poll++;
            const statusRes = await fetch(`${PARALLEL_BASE}/v1beta/findall/runs/${findall_id}`, { headers: findallHeaders, signal: AbortSignal.timeout(10_000) });
            if (!statusRes.ok)
                continue;
        ...
        const resultRes = await fetch(`${PARALLEL_BASE}/v1beta/findall/runs/${findall_id}/result`, { headers: findallHeaders, signal: AbortSignal.timeout(15_000) });
        if (!resultRes.ok) {
            log.error("Parallel FindAll result fetch error", { status: resultRes.status });
            return [];
        }
        const resultData = (await resultRes.json());
        const matched = (resultData.candidates ?? []).filter((c) => c.match_status === "matched");
 ...
 */
export async function routeCategoryQueries(queries) {
    ...
    const findAllObjective = `Find AI tools, projects, and startups launched in the last ${SEARCH_FRESHNESS_DAYS} days by indie developers ` +
    ...
    const [searchResults, findAllResults, tavilyResults, exaResults] = await Promise.all([
        ...
        Promise.all(parallelSearchCategories.map((q) => parallelSearch(q.queries, q.category, 8))).then((r) => r.flat()),
        // Parallel FindAll: one job per category (polls up to 90s internally)
        Promise.all(parallelFindAllCategories.map(() => parallelFindAll(findAllObjective, 20))).then((r) => r.flat()),
        // Tavily: per-query
        config.tavily?.apiKey
            ? Promise.all(tavilyCategories.flatMap((q) => q.queries.map(async (query) => {
        ...
        config.exa?.apiKey
            ? Promise.all(exaCategories.flatMap((q) => q.queries.map(async (query) => {
 ...
 * Exa search wrapper
 */
export async function exaSearch(query, limit) {
 ...
 */
export async function tavilySearch(query, limit) {
