...
import { evaluateContentRelevance, detectContentType, ensureResearchRepresentation, } from "./evaluate.js";
...
export async function runDiscovery(existingUrls) {
    ...
    log.info(`Sources: ${monitorEvents.length} monitor events, ` +
        `${categoryResults.length} category results, ` +
        `${arxivPapers.length} arXiv papers, ` +
        `${showHNPosts.length} Show HN posts`, { runId });
    ...
    const s3Checks = await Promise.all(uniqueItems.map(async (discovered) => {
    ...
    const evalResults = await mapConcurrent(newItems, 5, async (discovered) => {
    ...
    log.info(`${finalSelection.length}/${newItems.length} items selected (${researchCount} research)`, { runId });
    ...
    const uploadResults = await mapConcurrent(finalSelection, 10, async ({ item: discovered }) => {
