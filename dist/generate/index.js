...
export async function generateNewsletter(dateOrDates, bucketOrPrevious, previousNewsletterArg, usedIdentifierSet // keys used in previous editions — excluded from this run
) {
    ...
    if (usedIdentifierSet && usedIdentifierSet.size > 0) {
        ...
        if (excluded > 0)
            log.info(`Hard-dedup: excluded ${excluded} articles already used in previous editions`);
    }
    loadTimer.end();
    if (entries.length === 0) {
    ...
    for (const story of selection.stories) {
        ...
        if (apiKeys.ideogram) {
            const imageUrl = await generateStoryImage(story.title, story.summary, apiKeys.ideogram);
            if (imageUrl) {
